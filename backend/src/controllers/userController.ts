...
module.exports.invitationResponse_post = async(req: Request, res: Response) => {

    const { response, user, invitationId, project } = req.body

    try {

        // check if the user being invited is the currently signed in user
        if (!(user.id === res.locals.jwtUser.id)) throw "can't accept an invitation for someone else!"
